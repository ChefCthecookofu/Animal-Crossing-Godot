...
func _process(delta: float):
	...
	if sky:
		...
		# Blend between twilight and midnight
			...
			top_color = lerp(twilight_top_color, night_top_color, blend_factor / 0.5)
			horizon_color = lerp(twilight_horizon_color, night_horizon_color, blend_factor / 0.5)
		sky.sky_top_color = top_color
		sky.sky_horizon_color = horizon_color


		# Simulate sun intensity for day/night cycle
		var sun_energy_day = 1.0
		var sun_energy_night = 0.2
		sun.light_energy = lerp(sun_energy_night, sun_energy_day, clamp(cos(PI * norm_time_of_day) / 2 + 1, 0.0, 1.0))
